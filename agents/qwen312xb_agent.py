...
# Auto-detect model from vLLM Flight Deck
try:
...
client = OpenAI(base_url=BASE_URL, api_key="sk-4090")
...
def execute_bash(command: str) -> str:
    try:
        res = subprocess.run(command, shell=True, capture_output=True, text=True, timeout=30, env=os.environ)
...
def search_web(query: str) -> str:
    try:
        with DDGS() as ddgs:
            ...
            return "\n".join([f"{i+1}. {r['title']}: {r['body'][:200]}..." for i, r in enumerate(results)]) if results else "No results found."
    except Exception as e:
...
# [BYPASS MODE] Dynamically inject tool schema into the system prompt
...
def extract_qwen_tools(text):
    ...
    # Catch standard JSON tool_calls
    ...
    # Catch multi-parameter XML functions
    for m in re.finditer(r'<function=([^>]+)>(.*?)</function>', text, re.DOTALL):
        ...
        # Auto-cast integers for pagination if present
    ...
    while True:
        try:
            ...
            while True:
                ...
                # Failsafe JSON real-time correction
...
""", unsafe_allow_html=True)
    ...
    if submitted and user_input.strip() and not st.session_state.is_running:
        ...
        while True:
            ...
            full_content = full_content.replace('<arguments":', '"arguments":')
            t_list = [tool_calls_acc[i] for i in sorted(tool_calls_acc.keys())] if tool_calls_acc else []
