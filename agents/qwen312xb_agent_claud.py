...
def device_loading_context(module: torch.nn.Module,
    ...
    if target_device.type == 'vulkan':
        # ── ASAHI M1 MAX HANDSHAKE ──────────────────────────────────────────
        # Staged per-tensor Vulkan transfer with Z-ALPHA diagnostic probe.
        # Replaces bulk module.to('vulkan') to intercept BFloat16 / bad dtype
        # tensors before they kill the C++ Vulkan allocator.
        # ────────────────────────────────────────────────────────────────────
        for name, p in module.named_parameters():
            if p.device.type == 'cpu':
                original_device_states[name] = p.device

        print("\n[Z-ALPHA PROBE] Commencing individual Vulkan transfers...")

        for name, p in module.named_parameters():
            try:
                p.data = p.data.to('vulkan')
            except Exception as e:
                print(
                    f"❌ VULKAN PARAM CRASH: {name} "
                    f"| DTYPE: {p.dtype} "
        ...
        for name, b in module.named_buffers():
            try:
                b.data = b.data.to('vulkan')
            except Exception as e:
                print(
                    f"❌ VULKAN BUFFER CRASH: {name} "
                    f"| DTYPE: {b.dtype} "
        ...
        print("[Z-ALPHA PROBE] All Vulkan transfers OK.\n")

    else:
        # Standard CUDA / XPU / etc. path
        # Store original device states and move parameters to target device
        for name, p in module.named_parameters():
            if p.device.type == "cpu":
                original_device_states[name] = p.device
